...
var MoveUp = /** @class */ (function () {
    ...
    MoveUp.prototype.enable = function (state) {
        return utils_1.getParentNodeIndexFromState(state) >= 1;
    };
    MoveUp.prototype.onClick = function (_state, _dispatch, view) {
        var state = view.state;
        var rowNumber = utils_1.getParentNodeIndexFromState(state);
        var firstNode = utils_1.getRootNodeWithPosByIndex(state, rowNumber - 1);
        var secondNode = utils_1.getRootNodeWithPosByIndex(state, rowNumber);
        if (firstNode) {
            ...
            view.dispatch(prosemirror_utils_1.setTextSelection(firstIndex2)(view.state.tr).scrollIntoView());
        }
    };
    return MoveUp;
}());
