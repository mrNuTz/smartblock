...
function getDecorations(_a) {
    ...
    var decorations = [];
    var blocks = prosemirror_utils_1.findBlockNodes(doc).filter(function (item) { return item.node.type.name === name; });
    ...
    function parseNodes(nodes, className) {
        ...
        return nodes.map(function (node) {
            var classes = __spreadArrays(className, (node.properties ? node.properties.className : []));
            if (node.children) {
